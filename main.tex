...
\author{
Hugo Pavy \email{hugo.pavy@etu.minesparis.psl.eu} \\ % student 1
Tom Mariani \email{tom.mariani68@gmail.com} % student 2
}
...
In fact by developing the left term, replacing $\lambda_{max}$, and using that $\norm{.}_1\ge \norm{.}_2$
...
= \frac{1}{2} (\norm{y}_2^2 + \norm{X\beta_{min}}_2^2)
\end{equation}

Or $\norm{X\beta_{min}}^2_2 > 0$ because by assumption $\|\beta_{min}\|_2 > 0$ (we can assume $X \not = 0_{\RR^{n\times p}}$ ),

We have the contradiction (3) as we wanted, $\|\beta_{min}\|_2 = 0$, and therefore $\beta_{min} = 0_{\RR^p}$
...
By using the convolution theorem, the Parseval theorem and padding with zero in order to equal the dimension, we have that:
\begin{equation}
    \norm{\mathbf{x}-\sum_{k=1}^K{\mathbf{z}_k*\mathbf{d}_k}}_2^2 + \lambda \sum_{k=1}^K\norm{\mathbf{z}_k}_1 = \norm{\hat{\mathbf{x}}-\sum_{k=1}^K{\hat{\mathbf{z}_k}\odot\hat{\mathbf{d}_k}}}_2^2 + \lambda \sum_{k=1}^K\norm{\hat{\mathbf{z}_k}}_1 
\end{equation}

Where $\hat{.}$ is the Fourier transform and $\odot$ is the component wise product.

With, 
$\beta = \begin{bmatrix} \hat{\mathbf{z}}_1 \\ \vdots \\ \hat{\mathbf{z}}_K \end{bmatrix}$, we have that 

\begin{equation}
\norm{\hat{\mathbf{x}}-\sum_{k=1}^K{\hat{\mathbf{z}_k}\odot\hat{\mathbf{d}_k}}}_2^2 + \lambda \sum_{k=1}^K\norm{\hat{\mathbf{z}_k}}_1 = \norm{\hat{\mathbf{x}}-\sum_{k=1}^K{\hat{\mathbf{z}_k}\odot\hat{\mathbf{d}_k}}}_2^2 + \lambda \norm{\beta}_1
\end{equation}
Let's write:
\begin{equation}
D = \begin{bmatrix} \mathbf{\hat{d}_1}^T \\ \vdots \\ \mathbf{\hat{d}_K}^T \end{bmatrix}\in \mathbb{R}^{(K*N)}
\end{equation}

And $X = diag(D,..., D) \in \mathbb{R}^{(K.N)*(K.N)}$

Finally we have that:

\begin{equation}
\norm{\hat{\mathbf{x}}-\sum_{k=1}^K{\hat{\mathbf{z}_k}\odot\hat{\mathbf{d}_k}}}_2^2 + \lambda \sum_{k=1}^K\norm{\hat{\mathbf{z}_k}}_1 = \norm{\hat{\mathbf{x}}-X\beta}_2^2 + \lambda \norm{\beta}_1
\end{equation}

Where $X$ is the design matrix and $\beta$ is the response vector.

Finally, according to question 1, there exists $\lambda_{max}$ such that the sparse codes are only 0 for any $\lambda > \lambda_{max}$.

Because our sparse coding problem is a lasso regression, we have:
...
\begin{equation}
   ...
   = \mathbb{E}[\frac{1}{N}\sum_{n=0}^{N-\tau-1}X_n X_{n+\tau}] -\gamma(\tau)
\end{equation}

By linearity of the expectation:
\begin{equation}
   \mathbb{E}[\frac{1}{N}\sum_{n=0}^{N-\tau-1}X_n X_{n+\tau}] -\gamma(\tau)
   = \frac{1}{N}\sum_{n=0}^{N-\tau-1}\mathbb{E}[X_n X_{n+\tau}] -\gamma(\tau)
\end{equation}
...
Finally, the equation shows that we have a biased estimator but asymptotically unbiased. 

In order to de-bias the estimator, we could change $\hat{\gamma}$ into:
...
\begin{align}
   |J(f_k)|^2 
   &= J(f_k).J(f_k)^* \\
   &=\frac{1}{N}\sum_{n=0}^{N-1}\sum_{j=0}^{N-1}X_nX_je^{\frac{2ik(n-j)\pi}{N}}
\end{align}

This sum is the same as creating an array with N-1 raws and N-1 columns where the column j and raw n would be $X_iX_je^{\frac{2i(n-j)\pi}{N}}$

The sum we are computing is the sum of all the elements of this matrix. 
We can see that $N\hat{\gamma}(\tau)e^{\frac{2i(n-j)\pi}{N}}$ where $\tau=n-j$ will be the sum of the elements of the diagonals going from the coordinate (n, 0) to the coordinate (N, N-n). 
Same when we are going from (n, 0) up until (N-n, N) following the diagonal. 

With $\gamma(-\tau) = \gamma(\tau)$, at the end our sum is equal to:
\begin{align}
|J(f_k)|^2 
& = \frac{1}{N}\sum_{h=1-N}^{N-1} N\hat{\gamma}(h)e^{\frac{2ikh\pi}{N}}\\
&=\sum_{h=1-N}^{N-1} \hat{\gamma}(h)e^{\frac{2ikh\pi}{N}}
\end{align}

Then, we have $f^{(N)}= argmin_{f_k}(|f-f_k|r)$

We have $f^{(N)}\to f$, $\sum_{\tau\in\mathbb{Z}} |\gamma(\tau)| < \infty$ and because $\hat{\gamma}(\tau)$ is asymptotically unbiased, we have:
...
Thus $|J(f^{(N)})|^2$ is an asymptotically unbiased estimator of $S(f)$
\end{solution}
...
\begin{figure}
    ...
    \centerline{\includegraphics[width=\textwidth]{images/autocovariance_200.png}}
    \centerline{Autocovariance ($N=200$)}
    \end{minipage}
    \begin{minipage}[t]{0.3\textwidth}
    \centerline{\includegraphics[width=\textwidth]{images/autocovariance_500.png}}
    \centerline{Autocovariance ($N=500$)}
    \end{minipage}
    \begin{minipage}[t]{0.3\textwidth}
    \centerline{\includegraphics[width=\textwidth]{images/autocovariance_1000.png}}
    \centerline{Autocovariance ($N=1000$)}
    \end{minipage}
    \vskip1em
    \begin{minipage}[t]{0.3\textwidth}
    \centerline{\includegraphics[width=\textwidth]{images/periodogram_200.png}}
    \centerline{Periodogram ($N=200$)}
    \end{minipage}
    \begin{minipage}[t]{0.3\textwidth}
    \centerline{\includegraphics[width=\textwidth]{images/periodogram_500.png}}
    \centerline{Periodogram ($N=500$)}
    \end{minipage}
    \begin{minipage}[t]{0.3\textwidth}
    \centerline{\includegraphics[width=\textwidth]{images/periodogram_1000.png}}
...
\begin{solution}
    ...
    \begin{itemize}
    \item The first thing to notice is that we are close to the theoretical solution of Q1, i.e the standard deviation for $\tau=0$, which is the case here $(\sigma = 1)$ 
    and 0 elsewhere. The second thing to notice is that the standard deviation decreases with the number of timepoints (or the sampling frequency depending on how we see it) 
    which is a good sign to approach $\gamma(\tau)$ with $\hat \gamma(\tau)$ 
    \item The periodogram seems quite consistent with the number of timepoints, average value of 1 and standard deviation of 1 also.
...
\begin{solution}
The cos and the sin are just scale factors, thus the mean is unchanged for A,B we have:
...
Then:
\begin{align*}
\text{Var}(A(f_k)) &= \mathbb{E}\left[A(f_k)^2\right] \\
&= \mathbb{E}\left[\left(\sum_{n=0}^{N-1} X_n \cos\left(\frac{2\pi kn}{N}\right)\right)^2\right] \\
&= \mathbb{E}\left[ \sum_{n=0}^{N-1} \sum_{m=0}^{N-1} X_n X_m \cos\left(\frac{2\pi kn}{N}\right) \cos\left(\frac{2\pi km}{N}\right)\right] \\
&=  \sum_{n=0}^{N-1} \sum_{m=0}^{N-1} \mathbb{E}[X_n X_m] \mathbb{E}\left[\cos\left(\frac{2\pi kn}{N}\right) \cos\left(\frac{2\pi km}{N}\right)\right] \quad \text{(hint)} \\
&= \sum_{n=0}^{N-1} \mathbb{E}[X_n^2] \mathbb{E}\left[\cos^2\left(\frac{2\pi kn}{N}\right)\right] \quad (\mathbb{E}[X_n X_m] \neq 0 \text{ for } n = m) \\
&= {\sigma^2}\sum_{n=0}^{N-1} \cos^2\left(\frac{2\pi kn}{N}\right) \\
&= \frac{\sigma^2}{2} \sum_{n=0}^{N-1} \left(1 + \cos\left(\frac{4\pi kn}{N}\right)\right) \\
&= \frac{\sigma^2}{2} \left(N + \text{Re}\left(\sum_{n=0}^{N-1} e^{\frac{4i\pi kn}{N}}\right)\right) \\
&=  \frac{\sigma^2N}{2}
\end{align*}
...
\begin{equation}
Var(B(f_k))=\frac{\sigma^2N}{2}
\end{equation}

Finally we have found that A and B are just the same Gaussians.

--

Then, we have $J(f) = \frac{1}{N}(A(f) + i B(f))$, thus:
\begin{equation}
|J(f_k)|^2 = \frac{1}{N^2}(A(f_k)^2 + B(f_k)^2)
\end{equation}

Let's compute:

\begin{align}
\text{cov}(A(f_k), B(f_k)) &= \mathbb{E}\left[A(f_k) B(f_k)\right] \\
&= \mathbb{E}\left[ -\sum_{n=0}^{N-1} \sum_{m=0}^{N-1} X_n X_m \cos \left(\frac{2 \pi k n}{N}\right) \sin \left(\frac{2 \pi k m}{N}\right) \right] \\
&= -\sum_{n=0}^{N-1} \sum_{m=0}^{N-1} \mathbb{E}\left[X_n X_m\right] \mathbb{E}\left[\cos \left(\frac{2 \pi k n}{N}\right) \sin \left(\frac{2 \pi k m}{N}\right)\right] \quad (\text{hint}) \\
&= -\sum_{n=0}^{N-1} \mathbb{E}\left[X_n^2\right] \cos \left(\frac{2 \pi k n}{N}\right) \sin \left(\frac{2 \pi k n}{N}\right) \quad (\mathbb{E}[X_n X_m] \neq 0 \text{ for } n = m) \\
&= -\frac{\sigma^2}{2} \sum_{n=0}^{N-1} \sin \left(\frac{4 \pi k n}{N}\right) \\
&= -\frac{\sigma^2}{2} \text{Im} \left( \sum_{n=0}^{N-1} e^{\frac{4 i \pi k n}{N}} \right)\\
&= 0
\end{align}

Since \(A(f_k)\) and \(B(f_k)\) are independent Gaussian random variables, $|J(f_k)|^2$ is a chi squared distribution of rank 2.

-- 

We know that if \( X \sim \mathcal{N}(0, \sigma^2) \), then \( \mathbb{E}[X^4] = 3\sigma^4 \). We then have \(\text{Var}(X^2) = \mathbb{E}[X^4] - \mathbb{E}[X^2]^2 = 2\sigma^4\).

\begin{align}
\text{Var}(|J(f_k)|^2) &= \frac{1}{N^2} \left( \text{Var}(A(f_k)^2) + \text{Var}(B(f_k)^2) \right) \quad \text{as } A^2 \text{ and } B^2 \text{ are i.i.d.} \\
&= \frac{1}{N^2} \left( 2 \left( \sigma \sqrt{\frac{N}{2}} \right)^4 + 2 \left( \sigma \sqrt{\frac{N}{2}} \right)^4 \right) \quad \text{based on the previous remark} \\
&= \sigma^4
\end{align}

Furthermore, we can compute the expectation of \(|J(f_k)|^2\):

\begin{align}
\mathbb{E}[|J(f_k)|^2] &= \frac{1}{N} \left( \mathbb{E}[A(f_k)^2] + \mathbb{E}[B(f_k)^2] \right) \\
&= \frac{1}{N} \left( \text{Var}(A(f_k)) + \text{Var}(B(f_k)) \right) \\
&= \sigma^2
\end{align}

The distribution of \(|J(f_k)|^2\) doesn't depend on \( N \). This means that $\mathbb{E}(|J(f_k)|^2) $ and $\mathbb{V}(|J(f_k)|^2)$  are not changing with N, i.e. \(|J(f_k)|^2\) is not consistent.
...
\cos \frac{2\pi lt}{N}
\left( 
\mathbb{E}[X_n X_m] \mathbb{E}[X_s X_t] +
\mathbb{E}[X_n X_s] \mathbb{E}[X_m X_t] +
\mathbb{E}[X_n X_t] \mathbb{E}[X_m X_s]
\right)
\]

\noindent
The first term of the parenthesis leads to study cases where $n = m$ and $s = t$:
\[
\sum_{n,s}
\cos^2 \left( \frac{2\pi kn}{N} \right)
\cos^2 \left( \frac{2\pi ls}{N} \right)
\mathbb{E}[X_n^2] \mathbb{E}[X_s^2]
= \sigma^4 \sum_{n,s}
\cos^2 \left( \frac{2\pi kn}{N} \right)
\cos^2 \left( \frac{2\pi ls}{N} \right)
= \frac{\sigma^4 N^2}{4}
...
\noindent
The second term of the parenthesis leads to study cases where $n = s$ and $m = t$:
\[
\sum_{n,m}
\cos \frac{2\pi kn}{N}
\cos \frac{2\pi km}{N}
...
\[
= \frac{\sigma^4}{2} 
\sum_n \cos \frac{2\pi kn}{N} \cos \frac{2\pi ln}{N}
\sum_m \left[
\cos \frac{2\pi (l-k)m}{N} +
\cos \frac{2\pi (l+k)m}{N}
\right]
\]
...
\sum_n \cos \frac{2\pi kn}{N} \cos \frac{2\pi ln}{N}
\delta_{lk}
= \frac{\sigma^4 N^2}{4} \delta_{lk}
\quad 
\]


\noindent
The third and last term of the parenthesis leads to study cases where $n = t$ and $m = s$:
\[
\sum_{n,m}
\cos \frac{2\pi kn}{N}
\cos \frac{2\pi km}{N}
...
\cos \frac{2\pi lt}{N}
\mathbb{E}[X_n^2]\mathbb{E}[X_m^2]
= \frac{\sigma^4 N^2 \delta_{l,k}}{4}
\quad \text{(same method as 2nd term)}
\]
...
\[
\mathbb{E}[A(f_k)^2 A(f_l)^2]
= \frac{\sigma^4 N^2}{4} (1 + 2\delta_{l,k})
\]

\noindent
The same method gives us:
\[
\mathbb{E}[B(f_k)^2 B(f_l)^2]
= \frac{\sigma^4 N^2}{4} (1 + 2\delta_{l,k})
\]

\noindent
We can compute the covariance of $|J(f_k)|^2$ using what we did above:
\[
\operatorname{cov}(|J(f_k)|^2, |J(f_l)|^2)
= \mathbb{E}[|J(f_k)|^2 |J(f_l)|^2]
- \mathbb{E}[|J(f_k)|^2] \mathbb{E}[|J(f_l)|^2]
\]

\[
= \frac{1}{N^2} \mathbb{E}[A(f_k)^2 A(f_l)^2
+ B(f_k)^2 B(f_l)^2
+ A(f_k)^2 B(f_l)^2
+ B(f_k)^2 A(f_l)^2]
- \sigma^4
...
\text{i.e.} \quad
\operatorname{cov}(|J(f_k)|^2, |J(f_l)|^2)
= \sigma^4 (1 + \delta_{l,k}) - \sigma^4
\]

In particular, for $l \neq k$, 
$\operatorname{cov}(|J(f_k)|^2, |J(f_l)|^2) = 0$. 
As there is no correlation between the $|J(f_k)|^2$, 
\uline{it explains the randomness in the periodogram}. 
No pattern can be extracted from white noise.
...
\begin{figure}
    \centering
    \begin{minipage}[t]{0.3\textwidth}
    \centerline{\includegraphics[width=\textwidth]{images/barlett_200.png}}
    \centerline{Periodogram ($N=200$)}
    \end{minipage}
    \begin{minipage}[t]{0.3\textwidth}
    \centerline{\includegraphics[width=\textwidth]{images/barlett_500.png}}
    \centerline{Periodogram ($N=500$)}
    \end{minipage}
    \begin{minipage}[t]{0.3\textwidth}
    \centerline{\includegraphics[width=\textwidth]{images/barlett_1000.png}}
    \centerline{Periodogram ($N=1000$)}
    \end{minipage}
    \vskip1em
...
These graphs show a better standard deviation compared to Q6.
This is coherent with the estimation of Barlett, indeed, we choose $K=5$ and the standard deviation went from 1 to $1/\sqrt{5} \approx 0.4  $
...
\begin{figure}
    ...
    \centerline{\includegraphics[width=0.6\textwidth]{images/healty.png}}
    \centerline{Badly classified healthy step}
    \end{minipage}
    \vskip1em
    \begin{minipage}[t]{\textwidth}
    \centerline{\includegraphics[width=0.6\textwidth]{images/unhealty.png}}
